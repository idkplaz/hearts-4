...
public class GrowingHeartListener implements Listener {
    ...
    public void applyGrowth(Player p, PlayerData data) {
        ...
        // Base BLOCK_INTERACTION_RANGE in 1.21 is 4.5. We cap at 4.5 total.
        // Only apply negative modifier when shrunk, positive growth keeps vanilla 4.5.
        ...
        if (reach != null) {
            ...
            if (level < 0) {
                reach.addModifier(new AttributeModifier(REACH_UUID, "growing_reach", level * 0.1, AttributeModifier.Operation.ADD_NUMBER));
            }
        }
        AttributeInstance entityReach = p.getAttribute(Attribute.ENTITY_INTERACTION_RANGE);
        if (entityReach != null) {
            entityReach.getModifiers().stream().filter(m -> m.getUniqueId().equals(REACH_UUID)).findFirst().ifPresent(entityReach::removeModifier);
            if (level < 0) {
                entityReach.addModifier(new AttributeModifier(REACH_UUID, "growing_entity_reach", level * 0.1, AttributeModifier.Operation.ADD_NUMBER));
            }
        }
